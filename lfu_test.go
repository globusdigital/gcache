...
func TestLFUHas(t *testing.T) {
	gc := buildTestLoadingCacheWithExpiration[string, string](t, TYPE_LFU, 2, 10*time.Millisecond)
...
func TestLFUFreqListOrder(t *testing.T) {
	gc := buildTestCache[int, int](t, TYPE_LFU, 5)
	for i := 4; i >= 0; i-- {
		gc.Set(i, i)
		for j := 0; j <= i; j++ {
			gc.Get(i)
...
func TestLFUFreqListLength(t *testing.T) {
	...
	{
		gc := buildTestCache[string, string](t, TYPE_LFU, 5)
